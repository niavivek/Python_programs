...
def check_file(password):
	global comp_count
	comp_count = 0 # reset value to zero for next search
	...
	with open("common.txt","r") as common_file:
		...
		result = bin_search_rec(pass_lowercase, all_pass, 0, len(all_pass) - 1)
		if(result == True):
		...
		print("Number of comparisons = {}. Number of items in list = {}".format(comp_count, len(all_pass)))

#recursive method for binary search
def bin_search_rec(password, pass_list, min_index, max_index):
	"""
	...
	6. Call function recursively
	"""
	global comp_count
	comp_count += 1 # increase counter for comparison
	if(min_index > max_index):#if min index is > max index, all the indices have been searched
		return False
	mid_index = int((max_index + min_index)/2) #Compute the mid index average of max and min as an integer.
	if(pass_list[mid_index] == password):#If array[mid] equals password, search is complete 
		return True
	#If the mid value was too low, that is, array[mid] < password, then set min index = mid index + 1.
	elif(pass_list[mid_index] < password):
		return bin_search_rec(password,pass_list,mid_index + 1,max_index)
	#Otherwise, the mid index was too high. Set max index = mid index - 1.
	else:
		return bin_search_rec(password,pass_list,min_index,mid_index - 1)
